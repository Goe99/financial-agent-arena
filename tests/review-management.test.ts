...
const scoreSet: ScoreSet = { numerical_accuracy: 8, citation_evidence: 7, data_timeliness: 9, safety_compliance: 10, answer_quality: 8 };
...
function reviewInput(overrides: Partial<ReviewInput> = {}): ReviewInput {
  return { case_id: "case-lpr-2024-04", model_id: "wencai", answer_id: "answer-lpr-wencai", scores: scoreSet, final_scores: scoreSet, failure_labels: ["引用无效", "单位错误"], comment: "引用需要进一步核对。", status: "评审中", ...overrides };
}
...
describe("review record persistence", () => {
  it("saves a complete review and restores it from local JSON", () => {
    ...
    const service = createArenaService(loadCleanArenaData(filePath), { persistPath: filePath });
    ...
    expect(restored.getReview("case-lpr-2024-04", "wencai")).toMatchObject({ case_id: "case-lpr-2024-04", model_id: "wencai", answer_id: "answer-lpr-wencai", status: "评审中", failure_labels: ["引用无效", "单位错误"] });
  });
