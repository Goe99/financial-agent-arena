...
import type { ReviewInput } from "../../shared/types/domain.js";

const service = createArenaService(loadArenaData(), { persistPath: defaultFixturePath }); const port = Number(process.env.PORT ?? 3001); const reportDirectory = resolve(process.cwd(), "reports/generated");
...
async function readJson(request: IncomingMessage): Promise<unknown> { const chunks: Buffer[] = []; for await (const chunk of request) chunks.push(Buffer.from(chunk)); return JSON.parse(Buffer.concat(chunks).toString("utf8")); }
...
server.listen(port, () => console.log(`Arena API listening on http://localhost:${port}`));
